...
if (verInfo[0].checked) {
    ...
    butao.addEventListener('click', () => {
        if (numMes.value.length === 0 || dia.value.length === 0) {
        ...
        } else if (numMes.value > 12 || numMes.value <= 0 || dia.value > 30 || dia.value <= 0) {
            ...
            for (let contMes = 0; contMes < tamMes; contMes++) {
                solucao.innerHTML = `Desde o inicio do ano se passaram : <strong>${contMes * 30 + tamDia} dias</strong>`
    ...
    for (let contMes = 0; contMes < novoMes; contMes++) {
        solucao.innerHTML = `Desde o inicio do ano se passaram : <strong>${contMes * 30 + diaAtoma} dias</strong>`
    }
    solucao.style.visibility = "visible";
